...
def ocr_process():
    ...
    max_side = data.get('max_side') if data else None
    ...
    try:
        ...
        if img is not None:
            try:
                raw = img.tobytes()
                cache_key = hashlib.sha1(raw + str(target_text).encode('utf-8') + bytes([1 if use_angle_cls else 0])).hexdigest()
                if cache_key in _ocr_cache:
                    return jsonify({"code": 0, "data": _ocr_cache[cache_key]})
            except Exception:
                cache_key = None
        ...
        if cache_key and parsed_result is not None:
            _ocr_cache[cache_key] = parsed_result
